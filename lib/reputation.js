...
// Budgets chosen against what each service actually tolerates from one
// IP: Cloudflare throttles a single address somewhere around 10 req/s,
// and Safe Browsing quota is per-user-project rather than shared.
const sbLimiter = limiterFor("safeBrowsing", {
  concurrency: 4,
  minIntervalMs: 120,
  maxWaitMs: 4000,
});
// One budget per resolver, because the limit is enforced per server:
// a query to Cloudflare does not consume Quad9's allowance.
function dnsLimiter(endpoint) {
  return limiterFor("dns:" + new URL(endpoint).origin, {
    concurrency: 4,
    minIntervalMs: 110, // ~9/s, under Cloudflare's reported ~10/s
    // Generous, because nothing waits on this: the local verdict is
    // already decided and only the badge detail is being filled in.
    maxWaitMs: 9000,
  });
}
const sbBackoff = backoffFor("safeBrowsing", "google");
// Separate state for transport failures, which Google's curve does not
// describe: it speaks about HTTP responses, not an unreachable network.
const netBackoff = backoffFor("safeBrowsingNet", "gentle");
const dnsBackoff = backoffFor("dns", "gentle");
...
async function checkSafeBrowsing(rawUrl, settings) {
  ...
  // Google requires clients to stop asking after an error, so respect
  // that before doing any work.
  const waiting = Math.max(sbBackoff.blockedFor(), netBackoff.blockedFor());
  if (waiting > 0) {
    return {
      id: "safeBrowsing",
      checked: false,
      ...
      note:
        "Paused for " +
        (waiting < 60000
          ? Math.ceil(waiting / 1000) + " more second(s)"
          : Math.ceil(waiting / 60000) + " more minute(s)") +
  ...
  try {
    ...
    const res = await sbLimiter.run(() => fetchWithTimeout(
    ...
    if (!res.ok) {
      // Any non-200 puts us into the documented back-off curve.
      sbBackoff.fail(retryAfterMs(res.headers.get("Retry-After")));
      saveBackoffState();
      const detail =
    ...
    // A full queue is not a service failure, so it must not trigger the
    // back-off curve — it only means this check was skipped this time.
    if (err instanceof QueueBusy) {
      return {
        id: "safeBrowsing",
        checked: false,
        malicious: false,
        note: "Skipped — too many pages opened at once.",
      };
    }
    // Google's 15-minute curve is specified for a non-200 *response*.
    // Being offline or timing out is not the server rejecting us, so it
    // gets the gentle curve — otherwise one flaky moment would disable
    // the check for half an hour.
    netBackoff.fail();
    saveBackoffState();
    return {
      id: "safeBrowsing",
...
async function checkDnsBlocklist(rawUrl, settings) {
  ...
  const waiting = dnsBackoff.blockedFor();
  if (waiting > 0) {
    return {
      id: "dnsBlocklist",
      checked: false,
      malicious: false,
      note: "Paused briefly after an earlier resolver error.",
    };
  }
  ...
  const [cf, q9] = await Promise.allSettled([
    dnsLimiter(CLOUDFLARE_DOH).run(() => dnsResolve(CLOUDFLARE_DOH, hostname)),
    dnsLimiter(QUAD9_DOH).run(() => dnsResolve(QUAD9_DOH, hostname)),
  ]);

  // Being told to wait is not the same as being broken: report it as
  // skipped, and do not let it start the back-off curve.
  if ([cf, q9].some((x) => x.status === "rejected" && x.reason instanceof QueueBusy)) {
    return {
      id: "dnsBlocklist",
      checked: false,
      malicious: false,
...
async function checkDomainAge(rawUrl, settings) {
  ...
  const pausedFor = rdapPausedFor(registrable);
  if (pausedFor > 0) {
    return {
      id: "domainAge",
      checked: false,
      malicious: false,
      note: "Paused briefly after the registry asked us to slow down.",
    };
  }
