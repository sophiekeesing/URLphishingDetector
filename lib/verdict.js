...
// Rules that mean "we cannot see where this actually goes". These get
// their own flag rather than just a score, because hiding the
// destination defeats every other check: chaining one shortener behind
// another (ej.uz -> urlto.me -> real site) leaves nothing in the
// address to inspect, and the tracker in the middle still records you.
// Since resolving the chain would require making the request — which is
// the exact harm — the only real defence is to stop before it is sent.
const HIDES_DESTINATION = ["ipLogger", "shortener", "redirectorShape"];
...
export function evaluate(rawInput, { reputation } = {}) {
  ...
  const { signals: localSignals, official } = runHeuristics(url);

  const reputationMalicious = reputation?.malicious ?? false;
  const reputationReasons = reputation?.reasons ?? [];

  // Attackers do buy aged domains, so "this domain is old" must never
  // be allowed to cancel out evidence that it is imitating a brand.
  const impersonating = localSignals.some((s) =>
    ["typosquat", "lookalikeChars", "wrongTld", "combosquat", "brandSubdomain",
     "punycode", "homoglyph"].includes(s.id)
  );
  const reputationSignals = (reputation?.signals ?? []).filter(
    (s) => !(s.credit && impersonating)
  );

  const signals = [...localSignals, ...reputationSignals];

  // A confirmed blocklist hit maxes the score out; otherwise the
  // weighted signals add up.
  const finalScore = reputationMalicious
    ? 100
    : Math.max(0, Math.min(100, signals.reduce((sum, s) => sum + s.weight, 0)));
  ...
  return {
    ...
    reasons,
    signals,
    hidesDestination: signals.some((s) => HIDES_DESTINATION.includes(s.id)),
    href: url.href,
